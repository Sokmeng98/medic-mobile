...
class MyVideoEducation extends StatelessWidget {
  const MyVideoEducation({super.key});
  ...
  Widget build(BuildContext context) {
    return Consumer<DarkModeProvider>(builder: (context, darkModeProvider, _) {
      ...
      return MaterialApp(
        ...
        home: MyVideoEducationScreen(title: AppLocalizations.of(context)!.videEducation),
      );
    });
  }
}

class MyVideoEducationScreen extends StatefulWidget {
  const MyVideoEducationScreen({super.key, required this.title});

  final String title;

  @override
  State<MyVideoEducationScreen> createState() => _MyVideoEducationScreenState();
}

class _MyVideoEducationScreenState extends State<MyVideoEducationScreen> {
  bool loading = false;
  bool hasMore = true;
  int page = 1;
  int pageSize = 5;
  List<EducationItem> videoEducationAllData = [];
  List<EducationItem> videoEducationData = [];
  List<EducationItem> videoEducationDataSearch = [];
  ...
  void _search(String enteredKeyword) {
    debouncer.run(() {
      if (enteredKeyword.isEmpty) {
        loading = false;
        hasMore = true;
        videoEducationData = videoEducationDataSearch;
      } else {
        hasMore = false;
        videoEducationData = videoEducationAllData.where((article) => (article.title.toLowerCase().startsWith(enteredKeyword.toLowerCase()))).toList();
        if (videoEducationData.isEmpty) {
          loading = false;
        } else {
          loading = true;
        }
  ...
  Future fetchAllData() async {
    HomeService().getsEducationCount(1).then((dataCount) {
      HomeService().getsEducationSearch(1, dataCount).then((data) {
        setState(() {
          videoEducationAllData = data;
        });
      });
    });
  }
  ...
  Future fetchData() async {
    ...
    loading = true;
    HomeService().getsEducationCount(1).then((dataCount) {
      HomeService().getsEducation(1, page, pageSize).then((data) {
        setState(() {
          if (videoEducationData.length < dataCount) {
            page++;
            loading = false;
            videoEducationDataSearch.addAll(data);
            videoEducationData = videoEducationDataSearch;
          }
          if (videoEducationData.length == dataCount) {
            page--;
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        toolbarHeight: rlgScreen(context) ? 150.0 : 170.0,
        ...
        child: Center(
          child: Column(
            children: [
              ...
              if (videoEducationData.isNotEmpty)
                Expanded(
                  child: ListView.builder(
                    padding: EdgeInsets.symmetric(vertical: 10.0, horizontal: 20.0),
                    itemCount: videoEducationData.length + 1,
                    itemBuilder: (context, index) {
                      if (index < videoEducationData.length) {
                        EducationItem videoEducation = videoEducationData[index];

                        return Container(
                          margin: EdgeInsets.only(bottom: 20.0),
                          child: Article1Card(article: videoEducation),
                        );
                      } else {
                        return Padding(
